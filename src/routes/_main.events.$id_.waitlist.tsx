...
/**
 * /events/$id/waitlist — in the wings for an event.
 *
 * Stream-20 SCREEN-39. Sister to SCREEN-38. Phase 1 mocks queue position.
 * Phase 4 reads from a real event_waitlist table.
 */

export const Route = createFileRoute("/_main/events/$id_/waitlist")({
  head: () => ({ meta: [{ title: "Event · waitlist — COUPL" }] }),
  component: EventWaitlistScreen,
});

function EventWaitlistScreen() {
  const { id } = useParams({ from: "/_main/events/$id_/waitlist" });
  ...
  return (
    <EventsBackdrop>
      ...
      <header className="px-5 pt-2 pb-5">
        <p className="text-label-mono">Event · waitlist</p>
        <h1 className="mt-2 font-display text-[26px] leading-tight text-ink">
          In the wings for <em className="italic">{e.title}</em>.
        </h1>
      </header>
      ...
      <section className="px-5">
        ...
        >
          <div className="bg-gradient-to-b from-transparent to-ink/55 px-5 py-5">
            ...
            <p className="mt-1 flex items-center gap-1.5 font-body text-[12.5px] text-paper/85">
              <MapPin size={12} className="opacity-80" />
              {e.neighborhood} · with {e.host}
            </p>
          </div>
        </article>
      </section>
      ...
      <section className="px-5 pt-5">
        <article className="rounded-[18px] bg-paper px-5 py-5 shadow-elev-1">
          ...
          <p className="mt-2 font-body text-[13px] italic text-stone">
            Hosts release seats as RSVPs lapse. We'll write the moment one opens.
          </p>
        </article>
      </section>
