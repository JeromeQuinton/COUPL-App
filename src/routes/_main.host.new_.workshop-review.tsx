...
export const Route = createFileRoute("/_main/host/new_/workshop-review")({
  ...
  component: WorkshopReviewScreen,
});

const SUMMARY = [
  { label: "Title", value: "Tea & Honesty" },
  { label: "Opening prompt", value: '"What\'s a thing you\'ve stopped pretending about?"' },
  { label: "Duration", value: "90 minutes" },
  { label: "Seats", value: "12" },
  { label: "Ticket", value: "£18" },
  { label: "Date", value: "Thu 30 Oct · 19:30" },
  { label: "Venue", value: "Toconoco, London" },
];

function WorkshopReviewScreen() {
  ...
  return (
    <EventsBackdrop>
      <StatusBar
        leading={
          <Link to="/host/new/workshop-detail" aria-label="Back" className="ml-1 inline-flex h-7 w-7 items-center justify-center rounded-full text-ink/70 hover:bg-ink/5">
            <ChevronLeft size={18} />
          </Link>
        }
      />
        ...
        {SUMMARY.map((row) => (
          ...
          >
            ...
            <Link
              ...
              className="ml-4 shrink-0 text-label-mono text-plum-500 hover:text-plum-700"
            >
              Edit
            </Link>
          </li>
      ...
      <p className="px-5 pt-5 font-body text-[12.5px] italic text-stone">
        Once published, attendees can book within minutes. You can edit details up to 24h before.
      </p>
